...
#grobal
motoman_gp8 = moveit_commander.MoveGroupCommander("motoman_gp8")

# "jointPoint_1"
def jointPoint_1(jointPos):
    
    print ("jointPos.data[0] =", jointPos.data[0])
    joint_goal = motoman_gp8.get_current_joint_values()
    joint_goal[0] = jointPos.data[0]
    print ("manip:", motoman_gp8.go(joint_goal, wait=True))
...
def jointPoint_X(carPos):
    ...
    joint_5_b_waypoints = []
    joint_5_b_wpose = motoman_gp8.get_current_pose().pose
    joint_5_b_wpose.position.x = carPos.data[0]
    joint_5_b_waypoints.append(copy.deepcopy(joint_5_b_wpose))
    (plan, fraction) = motoman_gp8.compute_cartesian_path(joint_5_b_waypoints,0.01,0.0)
    print ("CARTESIAN",motoman_gp8.execute(plan, wait=True))
...
def jointPoint_Y(carPos):
    ...
    joint_1_s_waypoints = []
    joint_1_s_wpose = motoman_gp8.get_current_pose().pose
    joint_1_s_wpose.position.y = carPos.data[1]
    joint_1_s_waypoints.append(copy.deepcopy(joint_1_s_wpose))
    (plan, fraction) = motoman_gp8.compute_cartesian_path(joint_1_s_waypoints,0.01,0.0)
    print ("CARTESIAN",motoman_gp8.execute(plan, wait=True))
...
def jointPoint_Z(carPos):
    ...
    joint_3_u_waypoints = []
    joint_3_u_wpose = motoman_gp8.get_current_pose().pose
    joint_3_u_wpose.position.z = carPos.data[2]
    joint_3_u_waypoints.append(copy.deepcopy(joint_3_u_wpose))
    (plan, fraction) = motoman_gp8.compute_cartesian_path(joint_3_u_waypoints,0.01,0.0)
    print ("CARTESIAN",motoman_gp8.execute(plan, wait=True))
...
def arraycallback_jointPos(jointPos):
    ...
    # group name manip
    Motoman_gp8 = moveit_commander.MoveGroupCommander("motoman_gp8")
    #print ("manip Joint values:",Motoman_gp8.get_current_joint_values())
